<script lang="ts">
  import { Sun, Moon, Monitor, Sparkles, LogIn, Settings, Menu } from "lucide-svelte";
  ...
  const themeIcon = $derived(
    themeManager.theme === "system" ? Monitor : themeManager.isDark ? Moon : Sun
  );
  const themeLabel = $derived(
    themeManager.theme === "system" ? "System theme" : themeManager.isDark ? "Dark mode" : "Light mode"
  );
...
{#snippet branding(compact = false)}
  <div class="flex items-center gap-2">
    <div
      class="{compact ? 'w-9 h-9' : 'w-8 h-8'} rounded-xl bg-gradient-to-br from-primary to-secondary flex items-center justify-center shadow-md shadow-primary/20"
    >
      <Sparkles class="w-4 h-4 text-white" strokeWidth={2.5} />
    </div>
    <div class="flex flex-col">
      <h1 class="text-base font-bold font-display text-neutral tracking-tight leading-none">
        Chronos
      </h1>
      <p class="{compact ? 'text-[10px]' : 'text-[9px]'} font-bold uppercase tracking-widest text-neutral/40 leading-none mt-0.5">
        {dateInfo.fullDate}
      </p>
    </div>
  </div>
...
{#snippet statsDisplay(compact = false)}
  {#if stats.totalTasks > 0}
    ...
    <div
      class="flex items-center gap-2 bg-base-200/50 rounded-lg {compact ? 'px-2 py-1' : 'px-3 py-1 border border-base-200/50'}"
    ...
    >
      <div
        ...
        role="progressbar"
      ></div>
      <span class="{compact ? 'text-[10px] font-bold' : 'text-xs font-medium'} text-neutral/70 tabular-nums">
...
{#snippet themeButton()}
  <button
    ...
    title={themeLabel}
  >
    <svelte:component this={themeIcon} class="w-4 h-4" strokeWidth={2} />
  </button>
{/snippet}
...
{#snippet authSection(compact = false)}
  {#if isAuthenticated}
    <div class="{compact ? 'scale-75 origin-right' : 'pl-1 ml-1 border-l border-base-200'}">
...
<header class="sticky top-0 z-30 w-full border-b border-base-200/50 bg-base-100/80 backdrop-blur-md {className}">
  <!-- DESKTOP -->
  <div class="hidden md:flex items-center justify-between h-14 w-full px-6 xl:px-10">
